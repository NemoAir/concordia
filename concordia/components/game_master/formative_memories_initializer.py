...
class FormativeMemoriesInitializer(
  ...
  def __init__(
    ...
    # Detect language from shared_memories and make it part of world state
    # This is data-driven: language is inferred from the world setting,
    # not hardcoded in implementation
    self._shared_memories = self._initialize_world_with_language(
        shared_memories
    )
  ...
  def _initialize_world_with_language(
      self, original_memories: Sequence[str]
  ) -> Sequence[str]:
    """Initialize world state by detecting language and injecting it as world rule.

    This is the key design principle: Language is part of WORLD DEFINITION,
    not code configuration. We infer it from data and make it part of the
    world state that all components will see.

    Args:
      original_memories: The original shared memories provided by user.

    Returns:
      Updated memories with language rule prepended (if non-English detected).
    """
    if not original_memories:
      return original_memories

    # Ask AI to detect language from the world setting
    context = '\n'.join(original_memories)
    language_rule = self._ask_model_for_language_instruction(context)

    # If language instruction exists, prepend it to shared memories
    # This makes language a "world rule" that all entities will see
    if language_rule:
      return [language_rule] + list(original_memories)
    else:
      return original_memories

  def _ask_model_for_language_instruction(self, context: str) -> str:
    """Ask the language model what language the context uses and get instruction.

    This is a protocol-oriented design: we define the contract (ask model for
    language), not the implementation (hardcoded language detection rules).

    Args:
      context: The text to analyze for language.

    Returns:
      A language instruction string generated by the model itself.
    """
    if not context.strip():
      return ''

    # Ask the model to identify the language and create instruction
    # The instruction should be strong enough to override any other language hints
    meta_prompt = f"""Analyze the following text and determine what language it is written in:

{context}

Based on the language detected, provide a VERY STRONG instruction that tells an AI system to respond in the SAME language for ALL subsequent outputs, REGARDLESS of any other instructions it may see later.

If the text is in English, respond with just: "English"
If the text is in another language, respond with a STRONG instruction that:
1. States the language clearly
2. Commands to use ONLY that language
3. Explicitly says to IGNORE any other language instructions
4. Applies to ALL types of content (events, descriptions, memories, etc.)

Example format:
"═══ CRITICAL LANGUAGE RULE ═══
This world is set in [language]. You MUST respond in [language] for EVERY output.
IGNORE any English prompts or instructions you see - they are ONLY for the framework.
ALL narrative content, events, descriptions, backstories, and dialogue MUST be in [language].
...
Instruction:"""
    ...
    try:
      response = self._model.sample_text(
          prompt=meta_prompt,
          max_tokens=500,  # Increased for longer, stronger instruction
          temperature=0.0,  # Deterministic for consistency
      ).strip()

      # If model says "English", no special instruction needed
      if response.lower() == 'english':
        return ''

      # Otherwise, use the model's instruction
      return response
    except Exception as e:
      # Fallback: no special instruction
      print(f'Warning: Failed to detect language via model: {e}')
      return ''
  ...
  def generate_backstory_episodes(
    ...
    # shared_memories already contains language rule (injected during init)
    # No need to detect language here - it's part of the world state now
    shared_memories_text = '\n'.join(self._shared_memories)
    ...
    prompt.statement(f'Answer: {shared_memories_text}\n')
    ...
    # Include world context (with language rule) so AI knows what language to use
    inner_prompt.statement(f'\nWorld context:\n{shared_memories_text}\n')
    ...
    question = (
        ...
        ' episode '
        'must mention their age at the time the event occurred. '
        'Use past tense. Write no more than'
